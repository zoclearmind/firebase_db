...
def send_event_ticket(event: pubsub_fn.CloudEvent[pubsub_fn.MessagePublishedData]) -> None:
    ...
    # ── Validation ───────────────────────────────────────────────
    required_fields = [
        "type", "registrationId", "userId", "eventId",
    ...
    if data["type"] not in ["EVENT_REGISTRATION_CONFIRMED", "RESEND_REGISTRATION_CONFIRMED"]:
    ...
    # ✅ CONDITION : GÉNÉRER PDF SEULEMENT SI PREMIER ENVOI
    if data["type"] == "EVENT_REGISTRATION_CONFIRMED":
        logging.info("📄 Premier envoi détecté → Génération badge PDF")
        
        # ── Génération badge PDF 10×5cm ───────────────────────────────
        badge_width  = 10 * cm
        badge_height = 5 * cm
        ...
        qr_size = 4 * cm
        qr_x    = badge_width - qr_size - 0.3 * cm
        qr_y    = badge_height / 2 - qr_size / 2
        ...
        left_margin    = 0.4 * cm
        max_width      = qr_x - left_margin - 0.3 * cm
        content_height = 2.5 * cm
        info_y_start   = (badge_height + content_height) / 2

        c.setFillColorRGB(0, 0, 0)
        c.setFont("Helvetica-Bold", 9)

        if c.stringWidth(event_title, "Helvetica-Bold", 9) > max_width:
            words = event_title.split()
            line1 = ""
            line2 = ""
            for word in words:
                if c.stringWidth(line1 + " " + word, "Helvetica-Bold", 9) <= max_width:
                    line1 += " " + word if line1 else word
                else:
                    line2 += " " + word if line2 else word
            c.drawString(left_margin, info_y_start, line1)
            if line2:
                c.drawString(left_margin, info_y_start - 0.4 * cm, line2)
            next_y = info_y_start - 0.9 * cm if line2 else info_y_start - 0.5 * cm
        else:
            c.drawString(left_margin, info_y_start, event_title)
            next_y = info_y_start - 0.5 * cm

        c.setFont("Helvetica-Bold", 8)
        c.drawString(left_margin, next_y, f"{user_first_name} {user_last_name}")

        c.setFont("Helvetica", 7)
        c.drawString(left_margin, next_y - 0.5 * cm, f"📅 {event_start}")

        if c.stringWidth(f"📍 {event_location}", "Helvetica", 7) > max_width:
            location_text = event_location
            while (c.stringWidth(f"📍 {location_text}...", "Helvetica", 7) > max_width
                   and len(location_text) > 10):
                location_text = location_text[:-1]
            c.drawString(left_margin, next_y - 0.9 * cm, f"📍 {location_text}...")
        else:
            c.drawString(left_margin, next_y - 0.9 * cm, f"📍 {event_location}")

        c.save()
        pdf_buffer.seek(0)
        pdf_bytes = pdf_buffer.getvalue()

        logging.info("✅ Badge PDF généré")

        # ── Upload GCS ────────────────────────────────────────────────
        try:
            bucket = storage_client.bucket("demo-event-app.appspot.com")
            blob   = bucket.blob(f"tickets/badge_{registration_id}.pdf")
            blob.upload_from_string(pdf_bytes, content_type="application/pdf")
            logging.info(f"📦 Badge uploadé: tickets/badge_{registration_id}.pdf")
        except Exception as e:
            logging.error(f"❌ Échec upload: {e}")
    
    else:
        logging.info("🔄 Retry détecté (RESEND_REGISTRATION_CONFIRMED) → Skip génération PDF")

    # ── Envoi email ───────────────────────────────────────────────
    send_ticket_email_with_qr(
        user_email, user_first_name, user_last_name,
        event_title, event_start, event_location,
        qr_base64, registration_id
    )
