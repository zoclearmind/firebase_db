...
# AJOUT : Fonction centralisée d'envoi SMTP
...
# ║                                                              ║
# ║   Contient : routage Pub/Sub, validation, SMTP, envoi.      ║
...
def process_email(request):
    ...
    try:
        ...
        if "data" in pubsub_message:
            ...
            logging.error("Champ 'data' absent du message Pub/Sub")
            return ("Bad Request: data absent", 400)

    except Exception as e:
        logging.error(f"Erreur lors de la lecture des données : {e}")
        return (f"Bad Request: {e}", 400)

    logging.info(f"DONNÉES REÇUES (Type: {data.get('type')})")
    email_type = data.get("type")

    if not email_type:
        logging.error("Champ 'type' manquant dans le message")
        return ("Bad Request: type manquant", 400)

    email_field = ""
    
    # AJOUT : Support ACTIVATION_CODE
    if email_type == "ACTIVATION_CODE":
        email_field = data.get("email", "")
        required_fields = ["firstName", "code", "expiresAt"]
    
    # AJOUT : Support ACTIVATION_LINK
    elif email_type == "ACTIVATION_LINK":
        email_field = data.get("email", "")
        required_fields = ["userId", "firstName", "lastName", "default_password", "link"]
    
    # AJOUT : Support RESET_PASSWORD
    elif email_type == "RESET_PASSWORD":
        email_field = data.get("email", "")
        required_fields = ["firstName", "token", "expiresAt"]
    ...
    # AJOUT : Support EVENT_APPROVED
        ...
        email_field = data.get("email", "")
        required_fields = ["email", "firstName", "lastName", "default_password", "company_name", "link"]
        ...
        required_fields = ["companyName", "eventId", "token", "url"]
    else:
        logging.warning(f"Type d'email inconnu : {email_type}")
        return ("Bad Request: type inconnu", 400)

    missing = [f for f in required_fields if f not in data]
    if missing or not email_field:
        logging.error(f"Données incomplètes. Manquant : {missing}")
    ...
    try:
        ...
        # AJOUT : Dispatch ACTIVATION_LINK
        elif email_type == "ACTIVATION_LINK":
        ...
        logging.error(f"Échec de l'envoi pour {email_type} vers {email_field} : {str(e)}")
        return (f"Internal Server Error: {str(e)}", 500)


# ──────────────────────────────────────────────────────────────
# CONFIG SMTP (conservé pour compatibilité)
# ──────────────────────────────────────────────────────────────

def get_smtp_config():
    config = {
        "host": os.environ.get("SMTP_HOST", "smtp.zeptomail.com"),
        "port": int(os.environ.get("SMTP_PORT", "587")),
        "user": os.environ.get("SMTP_USER", "noreply@athena-event.com"),
        "pass": os.environ.get("SMTP_PASSWORD"),
    }
    if not config["user"] or not config["pass"]:
        raise ValueError("Configuration SMTP incomplète (User/Pass manquants)")
    return config
...
def send_activation_code(email: str, first_name: str, code: str, expires_at: str):
    rows = (
        _hero(
        ...
        + _info_card(
            _info_row("", "Expire dans", f"<strong>{expires_at} minutes</strong>"),
            label="EXPIRATION"
        )
        + _alert(
    ...
    msg.attach(MIMEText(
        ...
        f"Expire dans : {expires_at} minutes\n\n"
    ...
    _send_email(msg)
    logging.info(f"Email activation envoyé à {email}")


# AJOUT : Fonction send_hostess_activation_link
...
# AJOUT : Fonction send_reset_password_email
...
def send_participant_invitation_known(
    ...
    text_content = (
        ...
        f"Vous êtes invité à participer à un événement pour {company_name} "
        f"(Réf : #{event_id}).\n\n"
    ...
    msg['From']       = f"Athena Event <{conf['user']}>"
    msg['To']         = company_email
    msg['Reply-To']   = conf['user']
...
def send_participant_invitation_unknown(
    ...
    logging.info(f"send_participant_invitation_unknown: {company_email}")
    ...
    text_content = (
        ...
        f"Vous êtes invité à rejoindre un événement pour {company_name} "
        f"(Réf : #{event_id}).\n\n"
